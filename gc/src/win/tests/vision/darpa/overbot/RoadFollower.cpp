...
RoadFollower::RoadFollower() :
    m_angle(0.0), m_score(0.0), m_offset(0.0),
    m_ralph(0), m_offroad(0), m_sampler(0),
    m_debug(0), m_display(0), m_counter(0),
    m_display_image(0), m_threshold(0.0)
{
...
void RoadFollower::processFrame(IplImage* image) {
    ...
    if (m_display) {
        ...
        for (i=0; i<10; i++) {
            ...
            if (4*textsize.height < height/3) {
                // keep height of all text to less than 1/3 height of image
        ...
        if (m_score > m_threshold) {
            ...
            CvPoint pt2;
            pt2.x = width/2 + (int)(height*sin(angle*PI/180));
            pt2.y = (int)(height*cos(angle*PI/180));
