...
#include "oou.h" // Overbot/OpenSteer utilities
...
class Homunculus : public OpenSteer::SimpleVehicle
{
...
public:
    ...
    void update (const float currentTime, const float elapsedTime)
    {
        ...
        Vec3 steering;
        lastBraking = 0.0f;
        const bool offPath = !bodyInsidePath (getPath());
        if (stuck || offPath || detectImminentCollision ())
        {
            ...
            lastBraking = curvedSteering ? 3.0f : 2.0f; // QQQ
            applyBrakingForce (lastBraking, elapsedTime);
                                       ...
                                       const float elapsedTime)
    {
        ...
        collisionDetected = scanLocalXZRectangle (*getMap(), *this,
                                                  -halfWidth, halfWidth,
        ...
        if (demoSelect == 2)
        {
            ...
            if (! bodyInsidePath (getPath())) pathFollowOffTime += elapsedTime;
        }
    }
    ...
    Vec3 hintForObstacleAvoidance (void)
    {
        ...
        const Vec3 pathHeading = getPath()->tangentAt (p, pathFollowDirection);
        ...
        else
        {
            ...
            if (obstacle != Vec3::zero)
            {
                ...
                float outside;
                const Vec3 onPath = getPath()->mapPointToPath (obstacle, outside);
                ...
                if (outside < 0)
                {
                    ...
                    const int segmentIndex =
                        getPath()->indexOfNearestSegment (onPath);
                    const float segmentRadius = getPath()->radii [segmentIndex];
    ...
    // like steerToAvoidObstacles, but based on a ObTerrainMap indicating
    // the positions of impassible regions
    //
    ...
    {
        const float spacing = map.getCellDimensions () / 2;
        ...
        while (s < maxSide)
        {
            ...
            const int L = (curvedSteering ? 
                           (int) (scanObstacleMap (lOffset,
                                                   ...
                                                   lObsPos)
                                  / spacing) :
                           scanXZray (map, lOffset, step, maxSamples));
            const int R = (curvedSteering ? 
                           (int) (scanObstacleMap (rOffset,
                                                   ...
                                                   rObsPos)
                                  / spacing) :
                           scanXZray (map, rOffset, step, maxSamples));
            ...
            if ((L > 0) && (L < nearestL))
            {
                ...
                if (L < nearestR)
                {
                    nearestO = ((curvedSteering) ?
                                lObsPos :
                                lOffset + ((float)L * step));
                    collectLNOnarrowStats (s, nearestO);
                }
            }
            if ((R > 0) && (R < nearestR))
            {
                ...
                for (int j = -1; j < 2; j+=2)
                {
                    ...
                    const int scan = (curvedSteering ?
                                      (int) (scanObstacleMap (start,
                                                              ...
                                                              ignore)
                                             / spacing) :
                                      scanXZray (map, start, step, raySamples));
        ...
        if (hintGiven &&
            (minXXX ((float)nearestL, (float)nearestR) > (maxSamples * 0.8f)))
        {
    ...
    {
        // does nothing now, idea was that it might draw 2d text near vehicle
    ...
    // is the terrain at this global location definitely passable?
    //
    // XXX since this is called from a steering library candidate, it
    // XXX would be better for this to take a "map" argument, but it
    // XXX couldn't be const because passableCell is not. Leaving it
    // XXX like this for now.
    //
    bool passableTerrain (const Vec3& globalPosition) const
    {
        float units = getMap()->getCellDimensions ();
        float overbotX = convertVec3toOverbotX (globalPosition);
#ifndef NOT_OPENSTEERDEMO  // only when building OpenSteerDemo plug-in
        overbotX = - overbotX;
#endif // NOT_OPENSTEERDEMO
        float overbotY = convertVec3toOverbotY (globalPosition);
        int mapX = (int) round (overbotX / units);
        int mapY = (int) round (overbotY / units);
        return getMap()->passableCell (mapX, mapY);
    }


    // used to detect if vehicle body is on top of any obstacles
    //
    bool scanLocalXZRectangle (const AbstractTerrainMap& map,
                               const AbstractLocalSpace& localSpace,
                               ...
                               float zMin, float zMax) const
    {
        ...
        for (float x = xMin; x < xMax; x += spacing)
        {
            for (float z = zMin; z < zMax; z += spacing)
            {
                ...
                if (! passableTerrain (global)) return true;
            }
        }
        return false;
    }
    ...
    // the map for a "non-green" cell.  Returns the index of the first sample
    // whose cell is non-green, or zero if no hits found.
    int scanXZray (const AbstractTerrainMap& /* map */,
                   const Vec3& origin,
                   ...
                   const int sampleCount) const
    {
        ...
        for (int i = 1; i <= sampleCount; i++)
        {
            ...
            if (! passableTerrain (samplePoint)) return i;
        }
    ...
    bool detectImminentCollision (void)
    {
        ...
        const float spacing = getMap()->getCellDimensions () / 2;
        ...
        while (s < maxSide)
        {
            ...
            const int L = (curvedSteering ?
                           (int) (scanObstacleMap (lOffset + qqqLift,
                                                   ...
                                                   ignore)
                                  / spacing) :
                           scanXZray (*getMap(), lOffset, step, samples));
            const int R = (curvedSteering ?
                           (int) (scanObstacleMap (rOffset + qqqLift,
                                                   ...
                                                   ignore)
                                  / spacing) :
                           scanXZray (*getMap(), rOffset, step, samples));
    ...
    Vec3 predictFuturePosition (const float predictionTime) /* const */
    {
        if (curvedSteering)
        {
            ...
            annotationXZArcByLength (position (), center, arcLength, 20, 
                                     futurePositionColor);
            return prediction;
        }
        else
    ...
    // Pathway to OoGCRoute to use methods like indexOfNearestSegment and
    ...
    // true when in "stuck" state and (effectively) stopped
    bool stuckAndStopped (void)
    {
        const bool sas = stuck && (relativeSpeed () < 0.001f);
        if (sas) stuckCount++;
        return sas;
    }
    ...
    //
    bool bodyInsidePath (OoGCRoute* path)
    {
        if (demoSelect == 2)
        {
    ...
    Vec3 convertLinearToCurvedSpaceGlobal (const Vec3& linear)
    {
        ...
        {
            const Vec3 center = position () + localCenterOfCurvature;
            annotationXZArcByLength (position (), center,
                                     speed () * sign * -3, 20, gWhite);
        }
    ...
    float maxSpeedForCurvature ()
    {
        ...
        if (curvedSteering)
        {
            ...
            else
            {
                ...
                const Vec3 pathHeading =
                    getPath()->tangentAt (position (), pathFollowDirection);
    ...
    // Variation on drawXZArc -- 
    // uses annotation lines and measures by angle rather than arclength.
    //
    ...
    //
    void annotationXZArcByLength (const Vec3& start,
                                  const Vec3& center,
                                  const float arcLength,
                                  const int segments,
                                  const Vec3& color)
    {
        // determine angle corresponding to given arcLength
        const float twoPi = 2 * OPENSTEER_M_PI;
        const float radius = (start - center).length ();
        const float circumference = twoPi * radius;
        const float arcAngle = twoPi * arcLength / circumference;

        // draw arc
        annotationXZArcByAngle (start, center, arcAngle, segments, color);
    }


    // Variation on annotationXZArc:
    //     measures by angle rather than arclength.
    //
    // draw a circular arc on the XZ plane, from a start point, around
    // a center, for a given ANGLE, in a given number of segments and
    // color.  The sign of arcLength determines the direction in which
    // the arc is drawn.
    //
    // XXX this should eventually be in a library, make it a first
    // XXX class annotation queue, tie in with drawXZArc
    //
    void annotationXZArcByAngle (const Vec3& start,
                                 const Vec3& center,
                                 const float angle,
                                 const int segments,
                                 const Vec3& color) const
    {
        ...
        // determine the angular step per segment
        const float step = angle / segments;
...
#ifdef OBSOLETE
    // determine a safe distance along the current path which is obstacle
    // free, this is the value returned fromReactiveDriver::steer
    //
    float safeDistanceForMoveCommand (void)
    {
        // raw distance
        const float raw = distanceToNearestObstacleOnPath ();
        
        // discount this distance by 10% for safety
        const float adjusted = raw * 0.9;
        
        // adjust for half of vehicle length from center to front,
        // times two for safety
        const float margined = adjusted - (halfLength * 2.0f);

        // if the safe value (proportional to speed) we got above was
        // less than a half vehicle length, check the region just
        // ahead of the vehicle to see if we can creep forward.
        if ((margined < halfLength) &&
            (!scanLocalXZRectangle (*getMap (),
                                    *this,
                                    -1.0f - halfWidth,
                                    1.0f + halfWidth,
                                    0,
        ...
        return (margined > 0.0f) ? margined : 0.0f;
    }


    // estimate distance along path known to be obstacle-free
    // (XXX using qqqLNOnarrow (an offshoot of qqqLastNearestObstacle) is a
    // temporary hack -- better to measure distance along the path arc XXX)
    //
    float distanceToNearestObstacleOnPath (void)
    {
        if (qqqLNOnarrow != Vec3::zero)
        {
            // return distance from vehicle center to obstacle center
            // (XXX should it consider non-zero size of each? XXX)
            return (position () - qqqLNOnarrow).length();
        }
        else
        {
            // max distanced scanned
            return lookAheadTimeOA () * speed();
        }
    }
#endif // OBSOLETE
	//	New version per CWR email.
    // determine a safe distance along the current path which is obstacle
    // free, this is the value returned fromReactiveDriver::steer
    //
    float safeDistanceForMoveCommand (void)
    {
        ...
        const float maxScanDistance = lookAheadTimeOA () * maxSpeed ();
        const float safetyMargin = 0.5f; // on each side

        // width of curved corridor to scan
        const float halfScanWidth = halfWidth + safetyMargin;

        // scanning resolution
        const float step = getMap()->getCellDimensions () / 2;
        ...
        const float twoPi = 2 * OPENSTEER_M_PI;
        const float circumference = twoPi * arcRadius;
        const float arcAngle = maxScanDistance * sign * twoPi / circumference;
        const int segments = (int) (maxScanDistance / step);

        // keep track of nearest obstacle found for all scans
        float safeDistance = maxScanDistance;

        // save annotation state then turn it off
        const bool annStateBefore = enableAnnotation;
        enableAnnotation = false;

        // scan a set of arcs
        for (float x = -halfScanWidth; x <= halfScanWidth; x += step)
        {
            // find distance to first non-green map cell along this arc
            const Vec3 startPoint = position () + (side () * x);
            Vec3 ignore;
            const float distance =
                scanObstacleMap (startPoint, center, arcAngle, segments,
                                 0, gBlack, grayColor (0.5), ignore);

            // if this distance is nearest so far, save it
            // (0 means no obstacle found)
            if ((distance > 0.0f) && (safeDistance > distance))
            {
                safeDistance = distance;
            }
        }

        // restore anootation state
        enableAnnotation = annStateBefore;

        // make allowence for distance from center to front of vehicle
        const float allowance = safeDistance - halfLength;

        // always non-negative
        return (allowance < 0.0f) ? 0.0f : allowance;
    }


    void collectLNOnarrowStats (const float sideOffset,
                                const Vec3 nearestObstacle)
    {
        float margin = 0.5f;
        float threshold = halfWidth + margin;
        if (sideOffset < threshold) qqqLNOnarrow = nearestObstacle;
    }


    AbstractTerrainMap* getMap (void) const {return map;}
    AbstractTerrainMap* setMap (AbstractTerrainMap* _map) {return map = _map;}
    OoGCRoute* getPath (void) const {return path;}
    OoGCRoute* setPath (OoGCRoute* _path) {return path = (OoGCRoute*) _path;}

private:

    AbstractTerrainMap* map;
    OoGCRoute* path;
...
public:
    ...
    // record last braking deceleration
    // (allows the vehicle in OverbotPlugIn to mirror braking)
    float lastBraking;
