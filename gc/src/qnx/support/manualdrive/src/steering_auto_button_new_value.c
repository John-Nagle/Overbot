...
int
steering_auto_button_new_value( PtWidget_t *widget, ApInfo_t *apinfo, PtCallbackInfo_t *cbinfo )
	...
	PtGetResource( widget, Pt_ARG_ONOFF_STATE, &buttonval, 0 );	// get pointer to widget data
	bool ison = (*buttonval) != 0;							// true if on
	model->steering_auto_button_new_value(ison);		// do the work in the model
