...
ImageSampler::ImageSampler() :
    m_image(0), m_x1(0), m_x2(0), m_x3(0), m_x4(0), m_y1(0), m_y2(0), m_y3(0), m_y4(0),
...
const IplImage *ImageSampler::sample(IplImage *img) {
    ...
    for (yy=0; yy<m_height; yy++) {
        ...
        int ystep = y_iterator.getLastStepSize();
        ...
        for (xx=0; xx<m_image->width; xx++) {
            ...
            int xstep = x_iterator.getLastStepSize();
            if (xsample < 0) {
            ...
            if (xx < m_image->width && yy < m_image->height && xsample < img->width) {
                ...
                unsigned char green;
                unsigned char blue;
                getAveragePixel(img, xsample, y, xstep, ystep, &red, &green, &blue);
...
void ImageSampler::getAveragePixel(IplImage *img, int x, int y, 
    ...
    int delta_x = (int)((double)xstep/2.0);
    int delta_y = (int)((double)ystep/2.0);
    //if (delta_x < 1) delta_x = 1;
    //if (delta_y < 1) delta_y = 1;
    // loop over the y-coords
    for (int j=y-delta_y; j<=y+delta_y; j++) {
        ...
        for (int i=x-delta_x; i<=x+delta_x; i++) {
            if (i<0 || i>=img->width) {
                continue;
            }
            unsigned index = 3*i;
