...
//
//	Needs to be fully implemented, now that the map server DOES change the basepoint.
//
static void handle_basepoint(int rcvid, const GPSINSBasepoint& msg, Kalman_Filter* kf)
{
	...
	MsgError(rcvid, EOK);									// ***TEMP*** ***UNIMPLEMENTED***
	MsgError(rcvid, EINVAL);								// ***UNIMPLEMENTED***
}
//
...
static void handle_getbasepoint(int rcvid, const GPSINSGetBasepoint& msg, Kalman_Filter* kf)
{
	...
	//	***NOT SURE ABOUT CONVERSION HERE***
	//	***FRANK ZHANG will check***
