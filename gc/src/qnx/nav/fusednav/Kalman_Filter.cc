...
/* Was called from AHRS.handle_data()
   It's results are never used 8/3/05 D.Hull TODO: use or we can get rid of Kalman_Filter!
*/
void
...
/* Was called from GPS::handleBestXYZ()
   It's results are never used (8/3/05) D.Hull TODO: use or we can get rid of Kalman_Filter!
*/
void
Kalman_Filter::corrector(bool good)
{
	...
	clock_gettime(CLOCK_REALTIME, &tp);
	state.timestamp = timespec2nsec(&tp);
	ost::MutexLock lok2(mGPS.mLock);	// protect shared data with GPS
...
// Currently NOT USED 8/3/05 D.Hull - see fusednav's version for the real action
void
...
void
Kalman_Filter::makeAMatrix( Matrix<NUMSTATES,NUMSTATES> & A)
{
	A.fill();
